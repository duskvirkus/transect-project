# station-tab-ui Specification

## Purpose
Defines requirements for the station tab UI component rendered on the analysis page when the default dataset is active. The tab strip shows one tab per station, and the active tab's content panel displays the station's photo, name, population, a sourced blurb, and per-station answers to the 7 climatic controls.
